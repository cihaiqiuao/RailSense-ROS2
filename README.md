...
TrainROS/
  src/
    ...
    trainros_fusion/              # IMU/Laser 同步与轻量 Kalman 融合，发布 /train_state
...
- Fusion 节点使用 `message_filters::ApproximateTime` 同步 IMU/Laser，GPS 作为低频速度校正源，车钩检测作为最近状态缓存，并用轻量 Kalman Filter 融合 speed/acceleration 后以 50 Hz 发布 `/train_state`。
...
- fake serial 集成测试：自动创建三路 PTY，启动三个 driver、Fusion 和 Monitor，验证 `/imu/data`、`/gps/fix`、`/laser/scan`、`/train_state`、Kalman diagnostics 和延时 diagnostics 均有有效输出。
...
- 将当前轻量 Kalman 扩展为更完整的 EKF/多维状态估计，并完善 PSD/TSI 和 RViz 可视化。
